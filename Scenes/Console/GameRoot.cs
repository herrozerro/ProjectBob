...
public partial class GameRoot : Control
{
    ...
    private GameSim _gameSim;

    [Export] public int MaxScrollLines = 30;

    // Called when the node enters the scene tree for the first time.
    public override void _Ready()
    {
        _historyRows = (VBoxContainer)FindChild("HistoryRows");
        _scrollContainer = (ScrollContainer)FindChild("Scroll");
        _scrollBar = _scrollContainer.GetVScrollBar();
        _commandParser = (CommandParser)FindChild("CommandParser");
        
        _gameSim = FindChild("GameSim") as GameSim;
        
        _commandParser.Initialize(_gameSim);
    ...
    private void DisplayStartingMessage()
    {
        ...
        sb.AppendLine("Your directive is to explore the universe and catalog all stellar bodies.");
